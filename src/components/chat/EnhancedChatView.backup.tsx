...
const EnhancedChatView: React.FC<EnhancedChatViewProps> = ({
  ...
  const loadSolutions = async () => {
    try {
      ...
      // Ajouter des logs pour diagnostiquer le problème de réponse
      console.log('DEBUG EnhancedChatView - Response status:', response.status);
      console.log('DEBUG EnhancedChatView - Response headers:', Object.fromEntries(response.headers.entries()));
      
      if (!response.ok) {
        // Vérifier le type de contenu avant de parser comme JSON
        const contentType = response.headers.get('content-type');
        console.log('DEBUG EnhancedChatView - Response content type:', contentType);
        
        if (contentType && contentType.includes('application/json')) {
          const errorData = await response.json();
          throw new Error(errorData.error || 'Erreur lors de la récupération des solutions');
        } else {
          // Si ce n'est pas du JSON, récupérer le texte brut pour débogage
          const textResponse = await response.text();
          console.error('DEBUG EnhancedChatView - Non-JSON error response:', textResponse.substring(0, 200) + '...');
          throw new Error(`Erreur de serveur (${response.status}): Réponse non-JSON reçue`);
        }
      }
